...
int evolution_RR(FILE *data )
{
 ...
 // long unsigned n_ok,n_bad;                      // Counters for ODE Integrator
  ...
  double r_BL, cosTh_BL;                         // Radial and polar Boyer-Lindquist coordinates
...
/*----- Initializing  Variables and Memory Allocation -----*/
  ...
  n_kepler_max = N_periods*StepKT;//N_one_Kepler_period;            // Total Number of Time Steps within a Time Interval without applying Radiation Reaction Effects
    
  // Time interval for data saving
  //dt_sec = ceil(STORE_rate*Nfreq );
    
  dt_sec = ceil(DTsec*Nfreq );
...
/*---- Computing radial and polar Boyer-Lindquist's coordinates ----*/
      ...
      printf(" %lu  %4.6e   %4.6e   %4.6e \n",n_global,R_p_o, COSTH_p_o,PHI_OLD);

     // printf("b)r=%4.6e costh=%4.6e phi=%4.6e\n",R_p_o, PSI_NEW, COSTH_p_o, PHI_NEW);
      
/*----- ODE Geodesic Evolution -----*/
    geodesic_evolution(&PSI_NEW, &CHI_NEW, &PHI_NEW,&lambdat, t_initial, t_in_BHmass, DT_INI, DT_MIN, ACCURACY);
    // printf("PSI_NEW= %4.6e CHI_NEW=%4.5e\n",PSI_NEW,CHI_NEW);
    
      
/*----Saving Geodesic evolution----*/
    // if ((STORE_rate!= 0)&&(n_global%dt_sec==0))
    //  fprintf(data,"%4.6e   %4.6e   %4.6e   %4.6e",t_in_sec, lambdat,r_BL, cosTh_BL);
...
/**----- Compute the Time Evolution of the Constants of Motion -----**/
    compute_ELCQ_dot(P_p, ECCENTRICITY, INCLINATION,
	                   LZ, Q_CONSTANT,
	                   energy_0e, angular_momentum_z_0e, q_carter_0e,
...
/*----- Updating Old Values of the ODE Angles -----*/
    ...
    //printf("b)r=%4.6e  psi=%4.6e costh=%4.6e  chi=%4.6e\n",R_p_o, PSI_NEW, COSTH_p_o, CHI_NEW);
