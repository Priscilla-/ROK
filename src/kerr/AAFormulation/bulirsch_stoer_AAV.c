...
//int geodesic_RHS2(double *dxdt, double *x);


int geodesic_evolution_AV(double *, double*);
int osculating_RHS(double *,double*);

int midpoint_method_AAV(long unsigned n_half_steps, double dt_total, double *x, double *dxdt,double *x_predicted);
...
int bulirsch_stoer_AAV(double *x, double *dxdt, double *time, double dt_try, double tolerance, double *x_scale, double *dt_used, double *dt_next)
{
   ...
   for(;;)
   {
    for (k=1;k<=k_max;k++)
    {
      ...
      midpoint_method_AAV(k,dt,x_save,dxdt,x_estimated);
...
/*----- FIRST Stage -----*/
  if(MAG_SF == 0)
    ...
    osculating_RHS(z_minus_x,x_predicted);
  
  for (i = 1; i <= NVAR; i++)
    printf("z_minus_x[%d] =%4.6e,  x_predicted[%d] =%4.6e \n", i,z_minus_x[i],i,x_predicted[i]);
    
  for (i = 1; i <= NVAR; i++)
  {
    //dz_plus_x[i] = dt*dxdt[i];
...
/*----- GENERAL Step -----*/
  if(MAG_SF == 0)
    ...
    osculating_RHS(z_plus_x,x_predicted);

  for (i = 1; i <= NVAR; i++)
    printf("z_plus_x[%d] =%4.6e,  x_predicted[%d] =%4.6e \n", i,z_plus_x[i],i,x_predicted[i]);
